<div class="activities-dashboard dashboard-page-shell">
    ...
    <div class="dashboard-page-content space-y-6">
        <header class="module-accent-line dashboard-panel relative overflow-hidden">
            <div class="flex flex-col gap-3 bg-white px-3 py-3 sm:px-5 sm:py-5 lg:flex-row lg:items-center lg:justify-between">
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        <p class="text-xs font-bold uppercase tracking-[0.18em] text-slate-600">{{ __('Planification control center') }}</p>
                        {{-- <h1 class="text-2xl font-bold text-sky-950">Activities & milestones dashboard</h1> --}}
                        <h1 class="text-2xl font-bold text-blue-600">{{ __('Activities & milestones dashboard') }}</h1>
                        <p class="mt-1 text-sm text-slate-600">{{ __('Identify delayed commitments and projects that require corrective action.') }}</p>
                    </div>
                </div>
                <div class="flex flex-wrap items-center gap-2">
                    <span
                        class="rounded-lg border border-orange-200 bg-orange-50 px-3 py-2 text-xs font-bold text-orange-700">{{ trans_choice('ui.overdue_commitments', $metrics['overdue'] + $milestoneMetrics['overdue'], ['count' => $metrics['overdue'] + $milestoneMetrics['overdue']]) }}</span>
        ...
        <section class="dashboard-metrics-grid">
            @foreach ([
        ['label' => __('Total activities'), 'value' => $metrics['total'], 'accent' => 'border-blue-200', 'icon' => 'bg-blue-100 text-blue-700', 'path' => 'M9 5h6M9 9h6m-6 4h4m-7 8h12a2 2 0 0 0 2-2V5a2 2 0 0 0-2-2H6a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2Z'],
        ['label' => __('activity_control.completed'), 'value' => $metrics['completed'], 'accent' => 'border-emerald-200', 'icon' => 'bg-emerald-100 text-emerald-700', 'path' => 'm5 12 4 4L19 6'],
        ['label' => __('activity_control.overdue'), 'value' => $metrics['overdue'], 'accent' => 'border-red-200', 'icon' => 'bg-red-100 text-red-700', 'path' => 'M12 8v5m0 3h.01M10.3 3.9 2.4 18a2 2 0 0 0 1.75 3h15.7a2 2 0 0 0 1.75-3L13.7 3.9a2 2 0 0 0-3.4 0Z'],
        ['label' => __('On time'), 'value' => $metrics['pending'], 'accent' => 'border-cyan-200', 'icon' => 'bg-cyan-100 text-cyan-700', 'path' => 'M12 6v6l4 2m5-2a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z'],
        ['label' => __('Completion'), 'value' => $metrics['completion'] . '%', 'accent' => 'border-violet-200', 'icon' => 'bg-violet-100 text-violet-700', 'path' => 'M4 19V9m6 10V5m6 14v-7m4 7H2'],
    ] as $metric)
        ...
        <section class="dashboard-metrics-grid">
            @foreach ([['label' => __('Total milestones'), 'value' => $milestoneMetrics['total'], 'color' => 'text-blue-800', 'icon' => 'bg-blue-100 text-blue-700'], ['label' => __('Completed milestones'), 'value' => $milestoneMetrics['completed'], 'color' => 'text-emerald-700', 'icon' => 'bg-emerald-100 text-emerald-700'], ['label' => __('Overdue milestones'), 'value' => $milestoneMetrics['overdue'], 'color' => 'text-red-700', 'icon' => 'bg-red-100 text-red-700'], ['label' => __('Upcoming milestones'), 'value' => $milestoneMetrics['pending'], 'color' => 'text-cyan-700', 'icon' => 'bg-cyan-100 text-cyan-700'], ['label' => __('Milestone completion'), 'value' => $milestoneMetrics['completion'] . '%', 'color' => 'text-violet-700', 'icon' => 'bg-violet-100 text-violet-700']] as $metric)
        ...
        @if ($metrics['total'] > 0 || $milestoneMetrics['total'] > 0)
            <section class="rounded-xl border border-orange-200 bg-orange-50 p-4 shadow-sm">
                <h2 class="text-xs font-bold uppercase tracking-wide text-blue-700">{{ __('Management focus') }}</h2>
                        ...
                        <article wire:key="focus-{{ $activity->id }}" class="flex min-w-0 flex-col rounded-lg border border-orange-200 bg-white p-4 shadow-sm">
                            ...
                            <div class="mt-auto pt-4">
                                <span class="inline-flex rounded-full bg-orange-100 px-2.5 py-1 text-xs font-bold text-blue-800">
                                    {{ trans_choice('ui.days_overdue', $activity->days_overdue, ['count' => $activity->days_overdue]) }}
                                </span>
                            </div>
                        </article>
                    @empty
                        <p class="text-sm font-semibold text-sky-800 sm:col-span-2">{{ __('There are no overdue activities.') }}</p>
                    @endforelse
                </div>
            </section>
            <section class="activities-chart-grid grid gap-5 xl:grid-cols-2">
                <x-dashboard-chart-card class="activities-health-chart" title="{{ __('Activity health') }}"
                    filename="activity-health" subtitle="{{ __('Completed, overdue and upcoming activities') }}" height="27rem">
                    ...
                    @else
                        <div class="flex h-full flex-col items-center justify-center gap-2 text-center text-slate-500">
                            ...
                            <p class="text-sm font-semibold">{{ __('No activities available') }}</p>
                        </div>
                    @endif
                    <x-slot name="footer">{{ __('A growing red segment signals that planned work is not being closed on time.') }}</x-slot>
                </x-dashboard-chart-card>

                <x-dashboard-chart-card class="activities-health-chart" title="{{ __('Milestone health') }}"
                    filename="milestone-health" subtitle="{{ __('Completed, overdue and upcoming milestones') }}" height="27rem">
                    ...
                    @else
                        <div class="flex h-full flex-col items-center justify-center gap-2 text-center text-slate-500">
                            <span
                                class="flex h-12 w-12 items-center justify-center rounded-full bg-slate-100 text-xl">0</span>
                            <p class="text-sm font-semibold">{{ __('No milestones available') }}</p>
                        </div>
                    @endif
                    <x-slot name="footer">{{ __('Overdue milestones can affect the project sequence and should be reviewed before routine activities.') }}</x-slot>
                </x-dashboard-chart-card>

                <x-dashboard-chart-card class="activities-risk-chart xl:col-span-2"
                    title="{{ __('Projects requiring corrective action') }}" filename="overdue-commitments-by-project"
                    subtitle="{{ __('Combined overdue activities and milestones, ranked by operational exposure') }}"
                    height="32rem">
                    <x-dashboard-apex-chart :options="$riskProjectChart"
                        chart-key="activities-risk-projects-{{ md5(json_encode($riskProjectChart)) }}" />
                    <x-slot name="footer">{{ __('Projects at the top concentrate the largest overdue workload and should be reviewed first.') }}</x-slot>
                </x-dashboard-chart-card>

                <x-dashboard-chart-card title="{{ __('Eight-week execution trend') }}" filename="weekly-activity-trend"
                    subtitle="{{ __('Weekly movement of completed, overdue and upcoming work') }}" height="28rem">
                    <x-dashboard-apex-chart :options="$weeklyTrendChart"
                        chart-key="activities-weekly-trend-{{ md5(json_encode($weeklyTrendChart)) }}" />
                    <x-slot name="footer">{{ __('Compare blue versus orange each week to identify whether execution is recovering or deteriorating.') }}</x-slot>
                </x-dashboard-chart-card>

                <x-dashboard-chart-card title="{{ __('Overdue aging') }}" filename="overdue-activity-aging"
                    subtitle="{{ __('Time elapsed since incomplete activities became overdue') }}" height="28rem">
                    <x-dashboard-apex-chart :options="$agingChart"
                        chart-key="activities-aging-{{ md5(json_encode($agingChart)) }}" />
                    <x-slot name="footer">{{ __('Items in the 8+ week range should be escalated, rescheduled or formally closed.') }}</x-slot>
                </x-dashboard-chart-card>
            </section>
        @endif
        ...
        <section class="dashboard-panel overflow-hidden bg-white">
            <div class="soft-title-surface flex flex-wrap items-center justify-between gap-3 border-b px-5 py-4">
                <div>
                    <h2 class="font-bold text-sky-950">{{ __('Milestones requiring attention') }}</h2>
                    <p class="text-xs text-slate-500">{{ __('Overdue milestones first, followed by the nearest upcoming commitments') }}</p>
                </div>
                    ...
                    <div
                        ...
                        <div class="min-w-0">
                            <p class="break-words text-sm font-bold text-slate-800">
                                {{ $item->milestone?->name ?? ($item->milestone?->code ?? __('Milestone')) }}</p>
                        ...
                        <span class="text-xs font-semibold text-slate-500">{{ $item->due_date->translatedFormat('M Y') }}</span>
                        <span
                            class="{{ $isOverdue ? 'bg-orange-100 text-orange-800' : 'bg-sky-50 text-sky-700' }} rounded-full px-3 py-1 text-xs font-bold">
                            {{ trans_choice($isOverdue ? 'ui.days_overdue' : 'ui.days_remaining', abs((int) $days), ['count' => abs((int) $days)]) }}
                        </span>
                    </div>
                @empty
                    <p class="p-10 text-center text-sm text-slate-500">{{ __('No milestones require attention.') }}</p>
                @endforelse
            </div>
        ...
        <section class="grid gap-5 xl:grid-cols-[minmax(0,2fr)_minmax(280px,1fr)]">
            <div class="dashboard-panel bg-white p-5">
                <div class="flex flex-wrap items-center justify-between gap-3">
                    <div>
                        <h2 class="text-base font-bold text-sky-950">{{ __('Top projects by activities') }}</h2>
                        <p class="text-xs text-slate-500">{{ __('Execution distribution by project') }}</p>
                    </div>
                    ...
                    @forelse ($topProjects as $item)
                        <div class="rounded-lg border border-sky-100 bg-white p-3">
                            ...
                            <div class="mt-2 flex flex-wrap gap-x-4 gap-y-1 text-[11px] font-semibold text-slate-600">
                                <span>{{ $item['completed'] }} {{ __('completed') }}</span>
                                <span>{{ $item['overdue'] }} {{ __('overdue') }}</span>
                                <span>{{ $item['pending'] }} {{ __('upcoming') }}</span>
                            </div>
                        </div>
                    @empty
                        <p class="py-12 text-center text-sm text-slate-500">{{ __('No activities have been registered.') }}</p>
                    @endforelse
                </div>
            </div>
            ...
            <aside class="dashboard-panel overflow-hidden">
                <div class="border-b border-orange-100 bg-orange-50 px-5 py-4">
                    <h2 class="font-bold text-orange-900">{{ __('Status guide') }}</h2>
                </div>
                <div class="space-y-3 p-5 text-sm">
                    <div class="rounded-lg border border-sky-100 bg-sky-50 p-3"><b class="text-sky-900">{{ __('activity_control.completed') }}</b>
                        <p class="mt-1 text-slate-600">{{ __('Marked with a check in Planification.') }}</p>
                    </div>
                    <div class="rounded-lg border border-orange-200 bg-orange-50 p-3"><b
                            class="text-orange-900">{{ __('activity_control.overdue') }}</b>
                        <p class="mt-1 text-slate-600">{{ __('The planned week ended without execution.') }}</p>
                    </div>
                    <div class="rounded-lg border border-orange-100 bg-white p-3"><b
                            class="text-orange-800">{{ __('Upcoming') }}</b>
                        <p class="mt-1 text-slate-600">{{ __('Still within its planned date.') }}</p>
                    </div>
                </div>
            </aside>
        </section>
        ...
        <section id="activity-detail" class="dashboard-panel overflow-hidden" x-data x-on:activities-filtered.window="$el.scrollIntoView({ behavior: 'smooth', block: 'start' })">
            <div
                ...
                <div>
                    <h2 class="font-bold text-sky-950">{{ __('Activity detail') }}</h2>
            ...
            <div class="divide-y divide-sky-100">
                    ...
                    @php
                        $style = match ($activity->dashboard_status) {
                            'completed' => [
                                ...
                                'label' => __('activity_control.completed'),
                            ],
                            'overdue' => [
                                'marker' => 'bg-orange-500',
                                'badge' => 'bg-orange-100 text-orange-800',
                                'label' => __('activity_control.overdue'),
                            ],
                            default => [
                ...
                @empty
                    <p class="p-12 text-center text-sm text-slate-500">{{ __('No activities match these filters.') }}</p>
                @endforelse
            </div>
