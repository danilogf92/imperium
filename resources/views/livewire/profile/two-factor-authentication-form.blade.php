...
<section>
    <div class="flex items-start justify-between gap-4">
        <div>
            <div class="flex items-center gap-2">
                <h2 class="text-lg font-semibold text-slate-900">{{ __('Multi-factor authentication') }}</h2>
                <span @class([
                    ...
                    {{ $isEnabled ? __('Enabled') : ($isPending ? __('Pending confirmation') : __('Disabled')) }}
                </span>
            </div>
            <p class="mt-1 text-sm leading-6 text-slate-500">
                {{ __('Protect your account with a time-based code from your authenticator app.') }}
            </p>
        </div>
    ...
    @if (! $isEnabled && ! $isPending)
        <div class="mt-6 rounded-xl border border-blue-100 bg-blue-50/60 p-4 text-sm text-blue-900">
            {{ __('You will need an authenticator app such as Microsoft Authenticator, Google Authenticator or 1Password.') }}
        </div>

        <form wire:submit="enable" class="mt-5 space-y-4">
            <div>
                <x-input-label for="mfa_current_password_enable" :value="__('Current password')" />
            ...
            <x-primary-button>{{ __('Enable MFA') }}</x-primary-button>
        </form>
    @elseif ($isPending)
        <div class="mt-6 grid gap-6 lg:grid-cols-[220px_1fr]">
            ...
            <div>
                <ol class="space-y-3 text-sm text-slate-600">
                    <li><strong class="text-slate-800">1.</strong> {{ __('Scan the QR code with your authenticator app.') }}</li>
                    <li><strong class="text-slate-800">2.</strong> {{ __('Enter the generated 6-digit code below.') }}</li>
                    <li><strong class="text-slate-800">3.</strong> {{ __('Save the recovery codes after confirmation.') }}</li>
                </ol>

                <form wire:submit="confirm" class="mt-5">
                    <x-input-label for="mfa_confirmation_code" :value="__('Authentication code')" />
                    ...
                    <x-primary-button class="mt-4">{{ __('Confirm and activate') }}</x-primary-button>
                </form>

                <form wire:submit="disable" class="mt-5 border-t border-slate-200 pt-5">
                    <x-input-label for="mfa_current_password_cancel" :value="__('Current password to cancel setup')" />
                    <x-text-input wire:model="currentPassword" id="mfa_current_password_cancel"
                        type="password" class="mt-1 block w-full" autocomplete="current-password" />
                    <x-input-error :messages="$errors->get('currentPassword')" class="mt-2" />
                    <x-secondary-button type="submit" class="mt-3">{{ __('Cancel setup') }}</x-secondary-button>
                </form>
            </div>
        </div>
    @else
        <div class="mt-6 rounded-xl border border-emerald-200 bg-emerald-50 p-4">
            <p class="text-sm font-semibold text-emerald-800">{{ __('Your account is protected with MFA.') }}</p>
            <p class="mt-1 text-xs text-emerald-700">
                {{ __('You will be asked for an authentication code the next time you sign in.') }}
            </p>
        </div>
        ...
        <div class="mt-5 flex flex-wrap gap-3">
            <button type="button" wire:click="$toggle('showRecoveryCodes')"
                ...
                {{ $showRecoveryCodes ? __('Hide recovery codes') : __('Show recovery codes') }}
            </button>
        </div>
        ...
        @if ($showRecoveryCodes)
            <div class="mt-5 rounded-xl border border-amber-200 bg-amber-50 p-5">
                <div class="flex items-start gap-3">
                    ...
                    <div>
                        <p class="text-sm font-bold text-amber-900">{{ __('Store these codes in a secure place') }}</p>
                        <p class="mt-1 text-xs text-amber-800">{{ __('Each code can only be used once.') }}</p>
                    </div>
                </div>
        ...
        <div class="mt-6 grid gap-5 border-t border-slate-200 pt-6 md:grid-cols-2">
            <form wire:submit="regenerate" class="space-y-3">
                <x-input-label for="mfa_current_password_regenerate" :value="__('Current password')" />
            ...
            <form wire:submit="disable" class="space-y-3">
                <x-input-label for="mfa_current_password_disable" :value="__('Current password')" />
                <x-text-input wire:model="currentPassword" id="mfa_current_password_disable"
                    type="password" class="block w-full" autocomplete="current-password" />
                <x-input-error :messages="$errors->get('currentPassword')" />
                <x-danger-button>{{ __('Disable MFA') }}</x-danger-button>
            </form>
        </div>
    @endif

    <x-action-message class="mt-4" on="mfa-confirmed">{{ __('MFA enabled successfully.') }}</x-action-message>
    <x-action-message class="mt-4" on="mfa-disabled">{{ __('MFA disabled.') }}</x-action-message>
    <x-action-message class="mt-4" on="recovery-codes-regenerated">{{ __('Recovery codes regenerated.') }}</x-action-message>
</section>
