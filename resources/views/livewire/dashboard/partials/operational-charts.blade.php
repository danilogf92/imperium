<section aria-labelledby="dashboard-operations-title" class="space-y-4">
    <div class="soft-title-surface rounded-xl border px-5 py-4">
        <p class="text-xs font-bold uppercase tracking-[0.16em] text-emerald-700">{{ __('Operational control') }}</p>
        <h2 id="dashboard-operations-title" class="mt-1 text-lg font-bold text-slate-900">{{ __('Plants and suppliers') }}</h2>
        <p class="mt-1 text-sm text-slate-500">
            {{ __('Compare workload and capital by plant, then review purchasing concentration by supplier.') }}
        </p>
    </div>
