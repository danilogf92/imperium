@if ($pendingActivityDeleteId)
    <div wire:key="weekly-activity-delete-modal"
        role="dialog" aria-modal="true" aria-label="{{ __('Confirm deletion') }}" data-modal-open="true" class="app-modal fixed inset-0 z-[80] flex items-center justify-center p-2 sm:p-4"
        ...
        <div x-on:click.stop
            ...
            <div class="px-6 pb-5 pt-6 text-center">
                ...
                <h2 class="mt-4 text-lg font-bold text-slate-900">{{ __('Delete weekly activity?') }}</h2>
                <p class="mt-2 text-sm text-slate-600">{{ __('This activity will be permanently removed.') }}</p>
            ...
            <div class="flex flex-col-reverse gap-3 border-t border-slate-200 bg-slate-50 px-4 py-4 sm:flex-row sm:justify-end sm:px-6">
                ...
                <button type="button" wire:click="confirmDeleteWeeklyActivity" data-global-loading
                    ...
                    <span wire:loading.remove wire:target="confirmDeleteWeeklyActivity">{{ __('Delete activity') }}</span>
                    <span wire:loading wire:target="confirmDeleteWeeklyActivity">{{ __('Deleting...') }}</span>
                </button>
            </div>
        </div>
    </div>
