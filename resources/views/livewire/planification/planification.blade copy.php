<div class="min-h-screen bg-gray-100 py-6">
    <div class="mx-auto max-w-7xl space-y-6 px-4 sm:px-6 lg:px-8">
        <div>
            ...
            <p class="mt-1 text-sm text-gray-600">Build each project's timeline with the milestones configured in Admin.</p>
        </div>
        ...
        <div class="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm">
            <div class="flex flex-col gap-2 border-b border-gray-200 px-3 py-2 sm:flex-row sm:items-center sm:justify-between">
                <div class="flex min-w-0 flex-1 items-center gap-3">
                    ...
                    <div class="relative w-full sm:max-w-lg">
                        ...
                        @if (filled($search))
                        <button type="button" wire:click="clearSearch" title="Clear search" aria-label="Clear search"
            ...
            <div x-data="{ open: true }" class="border-b border-slate-200">
                <div class="flex items-center justify-between bg-gradient-to-r from-blue-50 via-white to-white px-5 py-3">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            <div class="flex items-center gap-2">
                                ...
                                @if (count($plantFilter) || count($statusFilter) || count($creationYearFilter) || $onlyWithMilestones || $currency !== 'usd' || $cellDisplay !== 'combined')
                                <span class="inline-flex items-center rounded-full bg-blue-600 px-2 py-0.5 text-[11px] font-semibold uppercase tracking-wide text-white">
                                    Active
                                </span>
                                @endif
                            </div>
                ...
                <div x-show="open" x-collapse>
                    <div class="flex items-center gap-3 overflow-x-auto bg-white px-4 py-4">
                        <x-dashboard-filter-dropdown label="Plants" model="plantFilter"
                            :options="$plantOptions->map(fn ($plant) => ['value' => $plant['id'], 'label' => $plant['name']])"
                            :selected="$plantFilter" multiple />

                        <x-dashboard-filter-dropdown label="Status" model="statusFilter"
                            :options="collect($statusOptions)->map(fn ($status) => ['value' => $status, 'label' => $status])"
                            :selected="$statusFilter" multiple />

                        <x-dashboard-filter-dropdown label="Creation Years" model="creationYearFilter"
                            :options="$creationYearOptions->map(fn ($year) => ['value' => $year, 'label' => $year])"
                        ...
                        <x-clear-filters-button method="clearFilters"
                            :active="$plantFilter !== [] || $statusFilter !== [] || $creationYearFilter !== [] || $onlyWithMilestones || $currency !== 'usd' || $cellDisplay !== 'combined'" />
                    </div>
                </div>
            </div>

            <div class="overflow-x-auto">
                <table class="table-fixed border-separate border-spacing-0" style="min-width: {{ 768 + ($timelineYears->count() * 1728) }}px">
                    <thead class="sticky top-0 z-20 shadow-sm">
                        <tr class="bg-indigo-600 text-white">
                            <th rowspan="2" class="sticky left-0 z-30 w-24 border-r border-indigo-500 bg-indigo-700 px-2 py-2 text-center text-xs font-semibold uppercase tracking-wide">
                                AÑO CREACIÓN
                            </th>
                            <th rowspan="2" class="sticky left-24 z-30 w-40 border-r border-indigo-500 bg-indigo-700 px-3 py-2 text-left text-xs font-semibold uppercase tracking-wide">
                                Planta
                            </th>
                            <th rowspan="2" class="sticky left-64 z-30 w-64 border-r border-indigo-500 bg-indigo-700 px-4 py-2 text-left text-xs font-semibold uppercase tracking-wide">
                                Nombre
                            </th>
                            <th rowspan="2" class="sticky left-[32rem] z-30 w-36 border-r border-indigo-500 bg-indigo-700 px-2 py-2 text-right text-xs font-semibold uppercase tracking-wide">
                                Budgeted total
                            </th>
                            <th rowspan="2" class="sticky left-[41rem] z-30 w-28 border-r-2 border-indigo-400 bg-indigo-700 px-2 py-2 text-center text-xs font-semibold uppercase tracking-wide">
                                Status
                            </th>
                        ...
                        <tr class="bg-indigo-500 text-white">
                            ...
                            <th class="w-36 border-r border-indigo-400 px-1 py-1.5 text-center text-xs font-semibold
                    ...
                    <tbody class="divide-y divide-gray-200">
                        ...
                        @php
                        $projectFirstYear = $plannedProject->forecast_start_date?->year
                        ?? $plannedProject->projectMilestones->min('cycle_year')
                        ?? now()->year;
                        ...
                        : null;
                        $projectClosed = $plannedProject->projectMilestones
                        ->contains(fn ($item) => strtoupper($item->milestone?->code ?? '') === 'CLOSED');
                        $projectBudget = (float) ($currency === 'eur'
                        ...
                        <tr wire:key="planned-project-{{ $plannedProject->id }}"
                            ...
                            <td class="sticky left-0 z-10 w-24 border-b border-r border-gray-200 px-2 py-1.5 text-center text-sm font-medium text-slate-700
                            {{ $loop->even ? 'bg-slate-50' : 'bg-white' }} group-hover:bg-indigo-50">
                                {{ $plannedProject->created_at?->year }}
                            </td>
                            <td class="sticky left-24 z-10 w-40 border-b border-r border-gray-200 px-3 py-1.5 text-sm text-slate-700
                            {{ $loop->even ? 'bg-slate-50' : 'bg-white' }} group-hover:bg-indigo-50">
                                <div class="truncate" title="{{ $plannedProject->company?->company_name }}">
                                    {{ $plannedProject->company?->company_name ?? '—' }}
                                </div>
                            </td>
                            <td class="sticky left-64 z-10 w-64 border-b border-r border-gray-200 px-4 py-1.5 align-middle
                            {{ $loop->even ? 'bg-slate-50' : 'bg-white' }} group-hover:bg-indigo-50">
                                <div class="flex items-center gap-2">
                                    <span class="inline-flex h-6 min-w-6 items-center justify-center rounded-md bg-indigo-100 px-1.5 text-xs font-bold text-indigo-700">
                                        {{ $plannedProject->id }}
                                    </span>
                                    <div class="line-clamp-1 text-sm font-medium text-gray-900" title="{{ $plannedProject->name }}">
                                        {{ $plannedProject->name }}
                                    </div>
                                </div>
                            </td>
                            <td class="sticky left-[32rem] z-10 w-36 border-b border-r border-gray-200 px-3 py-1.5 text-right text-sm font-bold text-slate-800
                            {{ $loop->even ? 'bg-slate-50' : 'bg-white' }} group-hover:bg-indigo-50">
                                {{ $currencySymbol }}{{ number_format($projectBudget, 2) }}
                            </td>
                            <td class="sticky left-[41rem] z-10 w-28 border-b border-r-2 border-gray-200 px-2 py-1.5 text-center
                            {{ $loop->even ? 'bg-slate-50' : 'bg-white' }} group-hover:bg-indigo-50">
                                @php
                            ...
                            @for ($monthNumber = 1; $monthNumber <= 12; $monthNumber++)
                                @php
                                $yearIsAvailable=in_array((int) $year, [$projectFirstYear, $projectFirstYear + 1], true);
                                $cellPosition=((int) $year * 12) + $monthNumber;
                                $cellCanCreate=! $projectClosed
                                && $yearIsAvailable
                                && ($firstPlannedPosition===null || $cellPosition>= $firstPlannedPosition);
                                @endphp
                                <td class="w-36 border-b border-r border-gray-200 px-0.5 py-0.5 text-center align-middle
                                    ...
                                    <div class="flex flex-nowrap justify-center gap-0.5 overflow-hidden">
                                        ...
                                        @php
                                        $milestoneValue = $projectBudget * ((float) $item->percentage / 100);
                                        $milestoneText = match ($cellDisplay) {
                                        'milestone' => $item->milestone->code,
                                        'value' => $currencySymbol . number_format($milestoneValue, 2),
                                        default => $item->milestone->code . ' | ' . $currencySymbol . number_format($milestoneValue, 2),
                                        };
                                        @endphp
                        ...
                        @empty
                        <tr>
                            <td colspan="{{ 5 + ($timelineYears->count() * 12) }}" class="px-5 py-12 text-center text-sm text-gray-500">No project plans found.</td>
                        </tr>
                        @endforelse
                    </tbody>
                </table>
    ...
    <div class="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 p-4 backdrop-blur-sm"
        ...
        <form wire:submit="saveMilestone" class="relative w-full max-w-lg overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-2xl">
            <div wire:loading.flex wire:target="saveMilestone"
                class="absolute inset-0 z-50 items-center justify-center bg-white/80 backdrop-blur-sm">
                <div class="flex items-center gap-3 rounded-xl border border-slate-200 bg-white px-5 py-4 shadow-xl">
                    ...
                    <span class="text-sm font-semibold text-slate-700">
                        {{ $editingId ? 'Saving changes...' : 'Creating milestone...' }}
                    </span>
                </div>
            </div>
            <div class="flex items-center justify-between border-b border-slate-200 bg-slate-50 px-6 py-4">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <h2 class="text-lg font-bold text-slate-900">{{ $editingId ? 'Edit milestone' : 'Add milestone' }}</h2>
                        <p class="text-xs text-slate-500">{{ $editingId ? 'Update its position in the project timeline.' : 'Place a milestone in the project timeline.' }}</p>
                    </div>
                </div>
            ...
            <div class="grid gap-4 px-6 py-5 sm:grid-cols-2">
                <div class="sm:col-span-2">
                    ...
                    <select wire:model.live="projectId" @disabled($editingId)
                ...
                <div class="sm:col-span-2">
                    ...
                    <select wire:model="milestoneId" class="block w-full rounded-lg border-gray-300 text-sm focus:border-indigo-500 focus:ring-indigo-500">
                        <option value="">Select a milestone</option>
                        @foreach ($milestones as $milestone)
                        <option value="{{ $milestone->id }}">{{ $milestone->code }} — {{ $milestone->name }}</option>
                        @endforeach
                    </select>
                    @error('milestoneId') <p class="mt-1 text-xs text-red-600">{{ $message }}</p> @enderror
                </div>
                <div>
                    <label class="mb-1 block text-sm font-medium text-gray-700">Month</label>
                    <select wire:model="month" class="block w-full rounded-lg border-gray-300 text-sm focus:border-indigo-500 focus:ring-indigo-500">
                ...
                <div>
                    ...
                    <input type="number" wire:model="cycleYear" min="2000" max="2200"
                        class="block w-full rounded-lg border-gray-300 text-sm focus:border-indigo-500 focus:ring-indigo-500">
                    @error('cycleYear') <p class="mt-1 text-xs text-red-600">{{ $message }}</p> @enderror
                ...
                <div class="sm:col-span-2">
                    ...
                    <div class="relative">
                        <input type="number" wire:model.live.debounce.250ms="percentage" min="0" max="100" step="0.01"
                    ...
                    $selectedBudget = (float) ($currency === 'eur'
                    ...
                    <p class="mt-1 text-xs text-slate-500">
                        Calculated value: {{ $currency === 'eur' ? '€' : '$' }}{{ number_format($previewValue, 2) }}
                    </p>
                </div>
            </div>
            <div class="flex justify-end gap-4 border-t border-slate-200 bg-slate-50 px-6 py-4">
                ...
                <button type="submit" wire:loading.attr="disabled" wire:target="saveMilestone"
                    ...
                    <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="{{ $editingId ? 'm5 13 4 4L19 7' : 'M12 4v16m8-8H4' }}" />
                    </svg>
                    <span wire:loading.remove wire:target="saveMilestone">{{ $editingId ? 'Save changes' : 'Create milestone' }}</span>
                    <span wire:loading wire:target="saveMilestone">{{ $editingId ? 'Saving...' : 'Creating...' }}</span>
                </button>
            </div>
        </form>
    </div>
    ...
    <div class="fixed inset-0 z-[60] flex items-center justify-center bg-slate-900/60 p-4 backdrop-blur-sm"
        ...
        <div class="w-full max-w-md overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-2xl">
            <div class="px-6 pb-5 pt-6 text-center">
                ...
                <p class="mt-2 text-sm text-slate-600">This milestone will be removed from the project timeline.</p>
                <p class="mt-3 rounded-lg bg-slate-50 px-3 py-2 text-sm font-semibold text-slate-800">{{ $pendingDeleteLabel }}</p>
            </div>
            <div class="flex justify-end gap-2 border-t border-slate-200 bg-slate-50 px-6 py-4">
                ...
                <button type="button" wire:click="confirmDeleteMilestone" wire:loading.attr="disabled"
