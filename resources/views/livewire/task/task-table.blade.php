        ...
        <section x-data="{ open: true }" class="rounded-xl border border-slate-200 bg-white shadow-sm">
            ...
            <div x-show="open" x-collapse>
                <div class="flex items-center gap-3 overflow-x-auto px-4 py-4">
                    ...
                    <x-dashboard-filter-dropdown label="Year" model="yearSearch"
                        :options="collect($years)->map(fn ($year) => ['value' => (string) $year, 'label' => (string) $year])"
                        :selected="$yearSearch" multiple />
                    <x-dashboard-filter-dropdown label="Supplier" model="supplierFilter"
                        :options="collect($supplierOptions)->map(fn ($value) => ['value' => $value, 'label' => $value])"
                        :selected="$supplierFilter" multiple />
                    <x-dashboard-filter-dropdown label="Order no." model="order_numberFilter"
                        :options="collect($orderNumberOptions)->map(fn ($value) => ['value' => $value, 'label' => $value])"
                        :selected="$order_numberFilter" multiple />
                    <x-dashboard-filter-dropdown label="PDA code" model="pda_codeFilter"
                        :options="collect($pda_Options)->map(fn ($value) => ['value' => $value, 'label' => $value])"
                        :selected="$pda_codeFilter" multiple />
                    <x-dashboard-filter-dropdown label="Status" model="statusFilter"
                        :options="collect($statusOptions)->map(fn ($value) => ['value' => $value, 'label' => ucfirst($value)])"
                        :selected="$statusFilter" multiple />

                    <x-per-page-select id="tasks-per-page" />

                    <x-clear-filters-button method="resetFilters"
                        :active="$search !== '' || $yearSearch !== [] || $supplierFilter !== [] || $order_numberFilter !== [] || $pda_codeFilter !== [] || $statusFilter !== []" />
                </div>
            </div>
        </section>
