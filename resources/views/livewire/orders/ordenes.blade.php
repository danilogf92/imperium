                    ...
                    <div>
                        <div class="flex items-center gap-2">
                            ...
                            <span
                                ...
                                {{ number_format($totalOrders) }} {{ __('orders') }}
                            </span>
                    ...
                    <div class="flex min-w-0 flex-1 flex-wrap items-center gap-3">
                        ...
                        <div class="relative w-full sm:max-w-lg">
                            ...
                            <input id="orders-search" wire:model.live.debounce.400ms="search" data-global-loading
                                type="text" placeholder="{{ __('Search order, project or PDA') }}" autocomplete="off"
                ...
                <table class="unified-data-table cursor-pointer">
                        ...
                        <tr>
                            ...
                            @if ($project)
                                <th wire:click="setSortBy('pda_code')" data-global-loading
                                    class="cursor-pointer px-4 py-3 transition hover:bg-blue-100 hover:text-blue-700">
                                    PDA @if ($sortBy === 'pda_code')
                                        {{ $sortDir === 'asc' ? '↑' : '↓' }}
                                    @endif
                                </th>
                                <th wire:click="setSortBy('plant')" data-global-loading
                                    class="cursor-pointer px-4 py-3 transition hover:bg-blue-100 hover:text-blue-700">
                                    Plant @if ($sortBy === 'plant')
                                        {{ $sortDir === 'asc' ? '↑' : '↓' }}
                                    @endif
                                </th>
                                <th wire:click="setSortBy('order_year')" data-global-loading
                                    class="cursor-pointer px-4 py-3 transition hover:bg-blue-100 hover:text-blue-700">
                                    Year @if ($sortBy === 'order_year')
                                        {{ $sortDir === 'asc' ? '↑' : '↓' }}
                                    @endif
                                </th>
                                <th wire:click="setSortBy('order_no')" data-global-loading
                                    class="cursor-pointer px-4 py-3 transition hover:bg-blue-100 hover:text-blue-700">
                                    <button type="button" class="font-semibold hover:text-blue-600">
                                        Order No. @if ($sortBy === 'order_no')
                                            {{ $sortDir === 'asc' ? '↑' : '↓' }}
                                        @endif
                                    </button>
                                </th>
                                <th wire:click="setSortBy('item_count')" data-global-loading
                                    class="cursor-pointer px-4 py-3 text-right transition hover:bg-blue-100 hover:text-blue-700">
                                    Items @if ($sortBy === 'item_count')
                                        {{ $sortDir === 'asc' ? '↑' : '↓' }}
                                    @endif
                                </th>
                            ...
                            @else
                                ...
                                <th wire:click="setSortBy('order_count')" data-global-loading
                                    class="cursor-pointer px-4 py-3 text-right transition hover:bg-blue-100 hover:text-blue-700">
                                    # Orders @if ($sortBy === 'order_count')
                                        {{ $sortDir === 'asc' ? '↑' : '↓' }}
                                    @endif
                                </th>
                                <th class="px-4 py-3 text-center">Actions</th>
                    ...
                    <tbody class="divide-y divide-slate-100">
                        @forelse ($orders as $order)
                            <tr wire:key="order-{{ $order->project_id }}-{{ $project ? $order->order_year . '-' . $order->order_no : 'project' }}"
                                x-on:click="if (!$event.target.closest('a, button, input, select, textarea, [role=button]')) window.location.href = @js(route('projects.orders', ['project' => $order->project_slug]))"
                                class="cursor-pointer text-slate-700 transition odd:bg-white even:bg-slate-50/50 hover:bg-blue-50/60">
                                @if (!$project)
                                    ...
                                    <td class="px-4 py-3 text-right">
                                        <span
                                            ...
                                            {{ number_format($order->order_count) }}
                                        </span>
                                    </td>
                                    <td class="px-4 py-3">
                                        <div class="flex justify-center gap-2">
                                            <a href="{{ route('projects.dashboard', ['project' => $order->project_slug]) }}"
                                                ...
                                                <svg class="h-4 w-4" viewBox="0 0 24 24" fill="none"
                                                    ...
                                                    <path stroke-linecap="round" stroke-linejoin="round"
                                                        d="M4.5 19.5h15M6.75 16.5v-4.5m5.25 4.5V8.25m5.25 8.25V5.25" />
                                                </svg>
                                            </a>
                                            <a href="{{ route('projects.orders', ['project' => $order->project_slug]) }}"
                                            ...
                                            <a href="{{ route('projects.data', ['project' => $order->project_slug]) }}"
                                                ...
                                                <svg class="h-4 w-4" viewBox="0 0 24 24" fill="none"
                                                    stroke="currentColor" stroke-width="1.8">
                                                    <path stroke-linecap="round" stroke-linejoin="round"
                                                        d="M12 3.75c4.142 0 7.5 1.343 7.5 3s-3.358 3-7.5 3-7.5-1.343-7.5-3 3.358-3 7.5-3Z" />
                                                </svg>
                                            </a>
                                        </div>
                                    </td>
