<div class="min-w-0" wire:key="handover-certificate-field{{ $fieldSuffix }}"
    ...
    <label for="handover-certificate{{ $fieldSuffix }}" class="mb-2 block text-sm font-medium text-gray-700">
        {{ __('Project Handover Certificate (PDF)') }}
    </label>

    @if ($editing && filled($currentHandoverCertificateName ?? null))
        <div class="mb-3 rounded-xl border border-violet-200 bg-violet-50 p-3">
            <p class="text-xs font-semibold uppercase tracking-wide text-violet-700">{{ __('Current handover certificate') }}</p>
            ...
            <button wire:click="downloadCurrentHandoverCertificate" wire:loading.attr="disabled"
                ...
                <span wire:loading.remove wire:target="downloadCurrentHandoverCertificate">{{ __('Download PDF') }}</span>
                <span wire:loading wire:target="downloadCurrentHandoverCertificate">{{ __('Preparing...') }}</span>
            </button>
        </div>
    @endif
    ...
    <label for="handover-certificate{{ $fieldSuffix }}"
        ...
        <span class="min-w-0 flex-1">
            <span class="block text-sm font-semibold text-slate-800">{{ filled($currentHandoverCertificateName ?? null) ? __('Replace Handover Certificate') : __('Upload Handover Certificate') }}</span>
            <span class="mt-1 block truncate text-xs text-slate-500">
                <span x-show="! uploading">{{ $handoverCertificate?->getClientOriginalName() ?? __('One PDF · maximum 10 MB') }}</span>
