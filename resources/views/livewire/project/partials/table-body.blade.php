    ...
    @forelse ($projects as $project)
        ...
        <tr wire:key="project-row-{{ $project->id }}"
            ...
            <td class="whitespace-nowrap px-2 py-2">
                <div class="flex items-center gap-2">
                    ...
                    @if (in_array((int) $project->company_id, $updateCompanyIds, true))
                        ...
                        @endif
                    @else
                        <span class="inline-flex h-8 w-8 items-center justify-center rounded-md text-white"
            ...
            <td class="whitespace-nowrap px-2 py-2 text-right font-semibold text-slate-700">
                {{ \App\Support\MoneyValueFormatter::compact((float) ($project->budgeted_euros ?? 0), '€ ') }}
            </td>

            {{-- Real Euros --}}
            <td class="whitespace-nowrap px-2 py-2 text-right font-semibold text-slate-700">
                {{ \App\Support\MoneyValueFormatter::compact((float) ($project->real_euros ?? 0), '€ ') }}
            </td>

            {{-- Executed Euros --}}
            <td class="whitespace-nowrap px-2 py-2 text-right font-semibold text-slate-700">
                {{ \App\Support\MoneyValueFormatter::compact((float) ($project->executed_euros ?? 0), '€ ') }}
            </td>

            {{-- Booked Euros --}}
            <td class="whitespace-nowrap px-2 py-2 text-right font-semibold text-slate-700">
                {{ \App\Support\MoneyValueFormatter::compact((float) ($project->booked_euros ?? 0), '€ ') }}
            </td>
            ...
            {{-- Budgeted Dollars --}}
            <td class="whitespace-nowrap px-2 py-2 text-right font-semibold text-slate-700">
                {{ \App\Support\MoneyValueFormatter::compact((float) ($project->budgeted_dollars ?? 0), '$ ') }}
            </td>

            {{-- Real Dollars --}}
            <td class="whitespace-nowrap px-2 py-2 text-right font-semibold text-slate-700">
                {{ \App\Support\MoneyValueFormatter::compact((float) ($project->real_dollars ?? 0), '$ ') }}
            </td>

            {{-- Executed Dollars --}}
            <td class="whitespace-nowrap px-2 py-2 text-right font-semibold text-slate-700">
                {{ \App\Support\MoneyValueFormatter::compact((float) ($project->executed_dollars ?? 0), '$ ') }}
            </td>

            {{-- Booked Dollars --}}
            <td class="whitespace-nowrap px-2 py-2 text-right font-semibold text-slate-700">
                {{ \App\Support\MoneyValueFormatter::compact((float) ($project->booked ?? 0), '$ ') }}
            </td>
            ...
            <td class="whitespace-nowrap px-2 py-2">
                ...
                <div class="flex items-center justify-center gap-1">
                    ...
                    @if (in_array((int) $project->company_id, $updateCompanyIds, true))
                        ...
                        <livewire:project.edit wire:key="edit-project-{{ $project->id }}" :project="$project" />
                    @endif

                    @if (in_array((int) $project->company_id, $deleteCompanyIds, true))
                        <livewire:project.delete wire:key="delete-project-{{ $project->id }}" :project="$project" />
                    @endif
