            ...
            <div class="flex min-h-full cursor-pointer items-center justify-center" x-on:click.self="$wire.close()">
                <section class="flex max-h-[calc(100dvh-1.5rem)] w-full max-w-lg cursor-default flex-col overflow-hidden rounded-xl bg-white shadow-2xl sm:max-h-[calc(100vh-3rem)]"
                    ...
                    <header class="flex shrink-0 items-start justify-between gap-4 border-b border-slate-200 bg-white p-4 sm:p-5">
                        <div class="min-w-0">
                            <h2 id="pda-modal-title" class="text-lg font-semibold text-slate-900">PDA document (PDF)</h2>
                            <p class="mt-1 truncate text-sm text-slate-500">{{ $projectCode }}@if ($projectName !== '') · {{ $projectName }}@endif</p>
                        </div>
                        <button wire:click="close" type="button" class="inline-flex h-9 w-9 shrink-0 cursor-pointer items-center justify-center rounded-lg text-slate-500 hover:bg-slate-100 focus:ring-2 focus:ring-blue-500" aria-label="Close modal">
                    ...
                    <div class="min-h-0 flex-[0_1_auto] overflow-y-auto bg-slate-50 p-4 sm:p-5">
                        @if ($currentDocumentName)
                            <div class="rounded-xl border border-emerald-200 bg-emerald-50 p-4">
                                <p class="text-xs font-semibold uppercase tracking-wide text-emerald-700">Current PDA</p>
                        ...
                        @else
                            <label for="pda-file-{{ $projectId }}" class="mb-2 block cursor-pointer text-sm font-semibold text-slate-700">Upload PDA</label>
                            ...
                            <label for="pda-file-{{ $projectId }}"
                                ...
                                <span class="min-w-0 flex-1">
                                    <span class="block text-sm font-semibold text-slate-800">Upload PDA PDF</span>
                                    <span class="mt-1 block truncate text-xs text-slate-500">
                                        <span x-show="! uploading">{{ $document?->getClientOriginalName() ?? 'Click to select one PDF · maximum 10 MB' }}</span>
                                        <span x-show="uploading" x-cloak class="font-semibold text-blue-700">Uploading PDA... <span x-text="`${progress}%`">0%</span></span>
                                    </span>
                                </span>
                        ...
                        @if ($deleteConfirmation)
                            <div class="mt-5 rounded-xl border border-red-200 bg-red-50 p-4">
                                <p class="font-semibold text-red-900">Delete this PDA PDF?</p>
                                <p class="mt-1 text-sm text-red-800">Only the PDA document will be removed.</p>
                                <div class="mt-3 flex justify-end gap-3">
                                    ...
                                    <button wire:click="delete" wire:loading.attr="disabled" type="button" class="cursor-pointer rounded-lg bg-red-600 px-4 py-2 text-sm font-semibold text-white hover:bg-red-700 disabled:cursor-wait disabled:opacity-60">Yes, delete PDF</button>
                                </div>
                            </div>
                        @endif
                    </div>
                    ...
                    <footer class="flex shrink-0 flex-col-reverse gap-3 border-t border-slate-200 bg-white p-4 sm:flex-row sm:justify-end sm:p-5">
                        ...
                        @elseif (! $currentDocumentName)
                            <button wire:click="uploadPdaDocument" wire:loading.attr="disabled" wire:target="uploadPdaDocument" type="button"
                                ...
                                <span wire:loading.remove wire:target="uploadPdaDocument">Upload PDA</span>
                                <span wire:loading wire:target="uploadPdaDocument">Saving PDA...</span>
                            </button>
                        @endif
                    </footer>
                </section>
