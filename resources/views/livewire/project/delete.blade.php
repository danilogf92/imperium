<div>
    ...
    <button type="button" wire:click="openModal" data-no-global-loading title="{{ __('Delete project') }}"
        ...
        <span class="sr-only">{{ __('Delete project') }}</span>
    ...
    <x-dialog-modal :name="$modalName" maxWidth="md" close-method="closeModal">
        ...
        <x-slot name="content">
            ...
            <p class="mt-3 text-sm font-medium text-red-600">
                {{ __('This deletes the entire project and its associated imported data. It is not the Project ideas file-only action.') }}
            </p>
        </x-slot>
        ...
        <x-slot name="footer">
            <div class="flex w-full flex-col-reverse gap-3 sm:flex-row sm:justify-end">
                ...
                <x-danger-button wire:click="deleteProject" data-global-loading wire:loading.attr="disabled"
                    wire:target="deleteProject">
                    <span wire:loading.remove wire:target="deleteProject">{{ __('Delete project') }}</span>
                    <span wire:loading wire:target="deleteProject">{{ __('Deleting...') }}</span>
                </x-danger-button>
            </div>
        </x-slot>
    </x-dialog-modal>
