    ...
    @if ($isOpen)
        <div data-modal-open="true" class="app-modal fixed inset-0 z-50 overflow-y-auto bg-slate-900/60 px-3 py-3 sm:py-6"
            ...
            <div class="flex min-h-full cursor-pointer items-center justify-center" x-on:click.self="$wire.close()">
                <section class="flex max-h-[calc(100dvh-1.5rem)] w-full max-w-2xl cursor-default flex-col overflow-hidden rounded-xl bg-white shadow-2xl sm:max-h-[calc(100dvh-3rem)]"
                    ...
                    <header class="flex shrink-0 items-start justify-between gap-4 border-b border-slate-200 bg-white p-4 sm:p-5">
                        <div class="min-w-0">
                            <h2 id="data-import-title" class="text-lg font-semibold text-slate-900">{{ __('Import project data from Excel') }}</h2>
                            <p class="mt-1 break-words text-sm text-slate-500">{{ $projectCode }}@if ($projectName !== '') · {{ $projectName }}@endif</p>
                        </div>
                        <button wire:click="close" type="button" class="inline-flex h-9 w-9 shrink-0 cursor-pointer items-center justify-center rounded-lg text-slate-500 hover:bg-slate-100 focus:ring-2 focus:ring-cyan-500" aria-label="{{ __('Close modal') }}">
                    ...
                    <div class="min-h-0 flex-[0_1_auto] overflow-y-auto bg-slate-50 p-4 sm:p-5">
                        <div class="rounded-xl border border-blue-200 bg-blue-50 p-4 text-sm text-blue-900">
                            {{ __('Complete the approved template without changing its column names. The ID column is ignored.') }}
                            <a href="{{ route('templates') }}" wire:navigate class="ml-1 cursor-pointer font-semibold underline underline-offset-2">{{ __('Download template') }}</a>
                        </div>

                        @if ($existingRows > 0)
                            <div class="mt-5 rounded-xl border border-amber-200 bg-amber-50 p-4">
                                <p class="font-semibold text-amber-900">{{ __('This project already has imported data') }}</p>
                                <p class="mt-1 text-sm text-amber-800">{{ __(':count rows exist. Delete those rows before importing a replacement workbook.', ['count' => number_format($existingRows)]) }}</p>
                            </div>
                        @else
                            <div class="mt-5">
                                ...
                                <label for="project-data-file-{{ $projectId }}"
                                    ...
                                    <span class="min-w-0 flex-1">
                                        <span class="block text-sm font-semibold text-slate-800">{{ __('Select completed project data Excel') }}</span>
                                        <span class="mt-1 block break-words text-xs text-slate-500">
                                            <span x-show="! uploading">{{ $file?->getClientOriginalName() ?? __('.xlsx or .xls · maximum 20 MB · 5,000 rows') }}</span>
                                            <span x-show="uploading" x-cloak class="font-semibold text-cyan-700">{{ __('Uploading...') }} <span x-text="`${progress}%`">0%</span></span>
                                        </span>
                                    </span>
                        ...
                        @if ($errors->has('dataImportErrors'))
                            <div class="mt-4 rounded-xl border border-red-300 bg-red-50 p-4">
                                <h3 class="font-semibold text-red-900">{{ __('Correct these cells and upload the workbook again') }}</h3>
                        ...
                        @if ($deleteConfirmation)
                            <div class="mt-5 rounded-xl border border-red-200 bg-red-50 p-4">
                                <p class="font-semibold text-red-900">{{ __('Delete :count imported data rows?', ['count' => number_format($existingRows)]) }}</p>
                                <p class="mt-1 text-sm text-red-800">{{ __('The project, PDA and Project ideas files remain unchanged.') }}</p>
                            </div>
                        @endif
                    </div>
                    ...
                    <footer class="flex shrink-0 flex-col-reverse gap-3 border-t border-slate-200 bg-white p-4 sm:flex-row sm:justify-end sm:p-5">
                        ...
                        @if ($existingRows > 0 && ! $deleteConfirmation)
                            <button wire:click="$set('deleteConfirmation', true)" type="button" class="cursor-pointer rounded-lg bg-red-600 px-4 py-2.5 text-sm font-semibold text-white hover:bg-red-700">{{ __('Delete imported rows') }}</button>
                        @elseif ($deleteConfirmation)
                            <button wire:click="deleteImportedRows" wire:loading.attr="disabled" type="button" class="cursor-pointer rounded-lg bg-red-600 px-4 py-2.5 text-sm font-semibold text-white disabled:cursor-wait disabled:opacity-60">{{ __('Yes, delete rows') }}</button>
                        @else
                            <button wire:click="importProjectDataWorkbook" wire:loading.attr="disabled" wire:target="importProjectDataWorkbook" type="button" class="cursor-pointer rounded-lg bg-cyan-600 px-4 py-2.5 text-sm font-semibold text-white hover:bg-cyan-700 disabled:cursor-wait disabled:opacity-60">
                                <span wire:loading.remove wire:target="importProjectDataWorkbook">{{ __('Import project data') }}</span><span wire:loading wire:target="importProjectDataWorkbook">{{ __('Validating and importing...') }}</span>
                            </button>
                        @endif
                    </footer>
                </section>
