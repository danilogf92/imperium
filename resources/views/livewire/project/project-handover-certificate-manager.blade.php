            ...
            <div class="flex min-h-full cursor-pointer items-center justify-center" x-on:click.self="$wire.close()">
                <section class="flex max-h-[calc(100dvh-1.5rem)] w-full max-w-lg cursor-default flex-col overflow-hidden rounded-xl bg-white shadow-2xl sm:max-h-[calc(100dvh-3rem)]"
                    ...
                    <header class="flex shrink-0 items-start justify-between gap-4 border-b border-slate-200 bg-white p-4 sm:p-5">
                        <div class="min-w-0">
                            <h2 id="handover-certificate-modal-title" class="text-lg font-semibold text-slate-900">{{ __('Project Handover Certificate (PDF)') }}</h2>
                    ...
                    <div class="min-h-0 flex-[0_1_auto] overflow-y-auto bg-slate-50 p-4 sm:p-5">
                        @if ($currentDocumentName)
                            <div class="rounded-xl border border-emerald-200 bg-emerald-50 p-4">
                                <p class="text-xs font-semibold uppercase tracking-wide text-emerald-700">{{ __('Current Project Handover Certificate') }}</p>
                            ...
                            @unless ($deleteConfirmation)
                                <div class="mt-4">
                                    <label for="handover-replacement-{{ $projectId }}" class="mb-2 block cursor-pointer text-sm font-semibold text-slate-700">{{ __('Replace certificate') }}</label>
                                    ...
                                    <label for="handover-replacement-{{ $projectId }}"
                                        ...
                                        <span class="min-w-0 truncate text-sm font-semibold text-slate-700">{{ $document?->getClientOriginalName() ?? __('Select a replacement PDF') }}</span>
                                        <span class="shrink-0 rounded-lg bg-violet-600 px-3 py-2 text-xs font-semibold text-white">{{ __('Choose file') }}</span>
                                    </label>
                                    @error('document')<p class="mt-2 text-sm font-medium text-red-600">{{ $message }}</p>@enderror
                                </div>
                            @endunless
                        @else
                            <label for="handover-file-{{ $projectId }}" class="mb-2 block cursor-pointer text-sm font-semibold text-slate-700">{{ __('Upload certificate') }}</label>
                            ...
                            <label for="handover-file-{{ $projectId }}"
                                ...
                                <span class="min-w-0 flex-1">
                                    <span class="block text-sm font-semibold text-slate-800">{{ __('Upload Project Handover Certificate') }}</span>
                                    <span class="mt-1 block break-words text-xs text-slate-500">
                                        ...
                                        <span x-show="uploading" x-cloak class="font-semibold text-violet-700">{{ __('Uploading...') }} <span x-text="`${progress}%`">0%</span></span>
                                    </span>
                                </span>
                        ...
                        @if ($deleteConfirmation)
                            <div class="mt-5 rounded-xl border border-red-200 bg-red-50 p-4">
                                <p class="font-semibold text-red-900">{{ __('Delete this Project Handover Certificate?') }}</p>
                                <p class="mt-1 text-sm text-red-800">{{ __('Only this PDF will be permanently removed.') }}</p>
                                <div class="mt-3 flex flex-wrap justify-end gap-3">
                                    ...
                                    <button wire:click="delete" wire:loading.attr="disabled" wire:target="delete" type="button" class="cursor-pointer rounded-lg bg-red-600 px-4 py-2 text-sm font-semibold text-white hover:bg-red-700 disabled:cursor-wait disabled:opacity-60">
                                        <span wire:loading.remove wire:target="delete">{{ __('Yes, delete PDF') }}</span>
                    ...
                    <footer class="flex shrink-0 flex-col-reverse gap-3 border-t border-slate-200 bg-white p-4 sm:flex-row sm:justify-end sm:p-5">
                        ...
                        @if (! $deleteConfirmation && $document)
                            <button wire:click="saveDocument" wire:loading.attr="disabled"
                                ...
                                <span wire:loading.remove wire:target="saveDocument">{{ $currentDocumentName ? __('Replace PDF') : __('Upload PDF') }}</span>
                                <span wire:loading wire:target="saveDocument">{{ __('Saving PDF...') }}</span>
                            </button>
                        @endif
                    </footer>
                </section>
