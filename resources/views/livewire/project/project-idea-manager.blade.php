            ...
            <div class="flex min-h-full cursor-pointer items-center justify-center" x-on:click.self="$wire.closeProjectIdeaModal()">
                <section class="flex max-h-[calc(100dvh-1.5rem)] w-full max-w-lg cursor-default flex-col overflow-hidden rounded-xl bg-white shadow-2xl sm:max-h-[calc(100dvh-3rem)]"
                    ...
                    <header class="flex shrink-0 items-start justify-between gap-4 border-b border-slate-200 bg-white p-4 sm:p-5">
                        <div class="min-w-0">
                            <h2 id="project-idea-title" class="text-lg font-semibold text-slate-900">{{ __('Project ideas (Excel)') }}</h2>
                    ...
                    <div class="min-h-0 flex-1 overflow-y-auto bg-slate-50 p-4 sm:p-5">
                        @if ($currentProjectIdeaFileName)
                            <div class="rounded-xl border border-emerald-200 bg-emerald-50 p-4">
                                <p class="text-xs font-semibold uppercase tracking-wide text-emerald-700">{{ __('Current Project ideas file') }}</p>
                                ...
                                <button wire:click="downloadProjectIdea" wire:loading.attr="disabled" wire:target="downloadProjectIdea" type="button"
                                    ...
                                    <span wire:loading.remove wire:target="downloadProjectIdea">{{ __('tools.download') }}</span>
                                    <span wire:loading wire:target="downloadProjectIdea">{{ __('Preparing...') }}</span>
                                </button>
                            </div>
                        @else
                            <div class="rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-800">{{ __('No Project ideas file has been uploaded.') }}</div>
                        @endif
                        ...
                        @if ($projectIdeaCanManage)
                            <div class="mt-5" wire:key="project-idea-upload-{{ $projectIdeaProjectId }}">
                                <label for="project-idea-file-{{ $projectIdeaProjectId }}" class="mb-2 block cursor-pointer text-sm font-semibold text-slate-700">
                                    {{ $currentProjectIdeaFileName ? __('Replace Excel file') : __('Upload Excel file') }}
                                </label>
                                ...
                                <p class="mt-2 text-xs text-slate-500">{{ __('One Excel file (.xlsx or .xls), maximum 10 MB.') }}</p>
                            ...
                            @if ($projectIdeaDeleteConfirmation)
                                <div class="mt-5 rounded-xl border border-red-200 bg-red-50 p-4">
                                    <p class="font-semibold text-red-900">{{ __('Delete this Project ideas Excel file?') }}</p>
                                    <p class="mt-1 text-sm text-red-800">{{ __('Only this file will be removed. The project and its imported project data remain unchanged.') }}</p>
                                    <div class="mt-3 flex flex-wrap justify-end gap-3">
                                        <button wire:click="cancelProjectIdeaDeletion" type="button" class="cursor-pointer rounded-lg border border-slate-300 bg-white px-4 py-2 text-sm font-semibold hover:bg-slate-50">{{ __('Keep file') }}</button>
                                        <button wire:click="deleteProjectIdea" wire:loading.attr="disabled" type="button" class="cursor-pointer rounded-lg bg-red-600 px-4 py-2 text-sm font-semibold text-white hover:bg-red-700 disabled:cursor-wait disabled:opacity-60">{{ __('Yes, delete Excel') }}</button>
                                    </div>
                                </div>
                            @endif
                        @endif
                    ...
                    <footer class="flex shrink-0 flex-col-reverse gap-3 border-t border-slate-200 bg-white p-4 sm:flex-row sm:flex-wrap sm:justify-end sm:p-5">
                        <button wire:click="closeProjectIdeaModal" type="button" class="cursor-pointer rounded-lg border border-red-700 bg-red-600 px-4 py-2.5 text-sm font-semibold text-white hover:bg-red-700">{{ __('Cancel') }}</button>
                        @if ($projectIdeaCanManage && $currentProjectIdeaFileName && ! $projectIdeaDeleteConfirmation)
                            <button wire:click="requestProjectIdeaDeletion" type="button" class="cursor-pointer rounded-lg bg-red-600 px-4 py-2.5 text-sm font-semibold text-white hover:bg-red-700">{{ __('Delete Excel') }}</button>
                        @endif
                        @if ($projectIdeaCanManage)
                            <button wire:click="saveProjectIdea" wire:loading.attr="disabled" wire:target="saveProjectIdea,projectIdeaFile" type="button"
                                ...
                                <span wire:loading.remove wire:target="saveProjectIdea">{{ $currentProjectIdeaFileName ? __('Replace Excel') : __('Upload Excel') }}</span>
                                <span wire:loading wire:target="saveProjectIdea">{{ __('Saving...') }}</span>
                            </button>
                        @endif
                    </footer>
                </section>
