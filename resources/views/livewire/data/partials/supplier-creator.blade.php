@if ($showSupplierCreator)
    <div data-modal-open="true" class="app-modal fixed inset-0 z-[190] flex items-center justify-center p-4" role="dialog"
        ...
        <button type="button" wire:click="toggleSupplierCreator" data-no-global-loading aria-label="{{ __('notes.close') }}"
        ...
        <div class="relative z-10 max-h-[calc(100dvh-2rem)] w-full max-w-lg overflow-y-auto overscroll-contain rounded-lg bg-white shadow-2xl">
            <div class="flex items-center justify-between border-b border-slate-200 px-5 py-4">
                <h3 id="new-supplier-title" class="text-base font-semibold text-slate-900">{{ __('Create supplier') }}</h3>
                <button type="button" wire:click="toggleSupplierCreator" data-no-global-loading aria-label="{{ __('notes.close') }}"
            ...
            <div class="space-y-5 px-5 py-5">
                ...
                <label class="block text-sm font-medium text-gray-700" for="new-supplier-name">{{ __('Name') }} <span class="text-red-500">*</span></label>
                <input id="new-supplier-name" type="text" wire:model="newSupplierName" maxlength="255"
                    placeholder="{{ __('Supplier name') }}" x-init="$nextTick(() => $el.focus())"
            ...
            <div class="flex flex-wrap justify-end gap-2 border-t border-slate-200 px-5 py-4">
                <button type="button" wire:click="toggleSupplierCreator" data-no-global-loading
                    class="rounded-md border border-gray-300 bg-white px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50">{{ __('Cancel') }}</button>
                <button type="button" wire:click="createSupplier" wire:loading.attr="disabled" wire:target="createSupplier" data-no-global-loading
                    ...
                    <span wire:loading.remove wire:target="createSupplier">{{ __('Create') }}</span>
                    <span wire:loading wire:target="createSupplier">{{ __('Creating...') }}</span>
                </button>
            </div>
        </div>
    </div>
