...
<button type="button"
    ...
    aria-label="{{ $active ? 'Clear all filters' : 'No filters applied' }}"
    {{ $attributes->class([
    ...
    <span wire:loading.remove wire:target="{{ $method }}">Clear filters</span>
    <span wire:loading wire:target="{{ $method }}">Clearing...</span>
</button>
