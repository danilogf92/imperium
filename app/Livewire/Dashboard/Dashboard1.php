...
class Dashboard extends Component
{
    ...
    public float|string $exchangeRate = 1.00;
    ...
    public function updatedExchangeRate(float|string $value): void
    {
        $this->exchangeRate = max((float) $value, 0.01);
    }
    ...
    public function render(): View
    {
        ...
        $totals = Data::query()
            ...
            ->selectRaw(
                'COALESCE(SUM(global_price_euros), 0) AS budgeted, '
                    ...
                    . 'COALESCE(SUM(real_value_euros), 0) AS real_value'
            )
            ->first();
        ...
        $budgetByInvestment = Data::query()
            ...
            ->selectRaw(
                'projects.investments AS label, '
                    . 'COALESCE(SUM(data.global_price_euros), 0) AS total'
            )
        ...
        $budgetByArea = Data::query()
            ...
            ->selectRaw(
                'area AS label, '
                    . 'COALESCE(SUM(global_price_euros), 0) AS total'
            )
            ->groupBy('area')
            ->orderByDesc('total')
            ->limit(10)
        ...
        $realValueByStartMonth = Data::query()
            ...
            ->selectRaw(
                $this->monthNumberExpression('projects.forecast_start_date')
                    . ' AS month, COALESCE(SUM(data.real_value_euros), 0) AS total'
            )
            ->groupBy('month')
            ->orderBy('month')
            ->pluck('total', 'month');
        ...
        return view('livewire.dashboard.dashboard', [
            ...
            'budgeted' => (float) $totals->budgeted * $multiplier,
            'booked' => (float) $totals->booked * $multiplier,
            'executed' => (float) $totals->executed * $multiplier,
            'realValue' => (float) $totals->real_value * $multiplier,
            ...
            $this->budgetColumnChart($budgetByInvestment, $multiplier),
            'budgetByStateChart' =>
            $this->budgetStateChart($budgetByState, $multiplier),
            'budgetByStateColumnChart' =>
            $this->budgetStateColumnChart($budgetByState, $multiplier),
            ...
            $this->budgetAreaRadarChart($budgetByArea, $multiplier),
            ...
            (float) $totals->real_value * $multiplier,
    ...
    private function currencyMultiplier(): float
    {
        return $this->currency === 'dollar'
            ? 1 / max($this->exchangeRate, 0.01)
            : 1;
    }
