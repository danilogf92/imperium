...
final class ExcelFilterValueConverter
{
    public function convert(string $value, string $type, string $header): array
    {
        ...
        if (! preg_match('/^[+-]?(?:\d{1,3}(?:[,.]\d{3})+|\d+)(?:[,.]\d+)?$/', $input)) {
            throw ValidationException::withMessages(['columnTypes' => __('tools.error_not_numeric', ['column' => $header, 'value' => $value])]);
        }
        ...
        if ($decimals > 10) {
            throw ValidationException::withMessages(['columnTypes' => __('tools.error_decimals', ['column' => $header])]);
        }
        ...
        if (! is_numeric($canonical) || strlen(ltrim(str_replace('.', '', $canonical), '+-0')) > 15) {
            throw ValidationException::withMessages(['columnTypes' => __('tools.error_precision', ['column' => $header, 'value' => $value])]);
        }
